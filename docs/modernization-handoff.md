...
Sample article visibility correction: the Latin example is now published in the seed and the local preview database, with its detail URL included in the review export. Existing draft content was not published. The sample is explicitly labeled as placeholder content, not attributed to a pastor. Article containers now have a 900px width limit. The shared page shell uses a viewport-height flex column so short pages leave no gap below the footer.
